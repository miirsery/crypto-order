...
const sizes = (screen: number) => {
  if (screen >= 1920) return 'lg'
  else if (screen >= 992) return 'md'
...
const isMobileView = (userAgent: string): boolean => {
  if (!userAgent) return false

  return REGEX_MOBILE1.test(userAgent) || REGEX_MOBILE2.test(userAgent.substr(0, 4))
}

const isMobileOrTabletView = (userAgent: string): boolean => {
  if (!userAgent) return false
...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  let screen: UnwrapNestedRefs<Screen>
  ...
  if (process.client) {
    screen = reactive<Screen>({
      size: sizes(innerWidth),
      width: innerWidth,
      isMobile: computed(() => ['xs'].includes(screen.size)),
      isTablet: computed(() => ['sm'].includes(screen.size)),
      isDesktop: computed(() => ['lg', 'xl'].includes(screen.size)),
      isFullHd: computed(() => ['xl'].includes(screen.size)),
      isMobileOrTablet: computed(() => ['xs', 'sm'].includes(screen.size)),
    })
    ...
    screen = reactive<Screen>({
      size: mobileOrTablet ? 'sm' : isMobile ? 'xs' : 'lg',
      width: mobileOrTablet ? 1439 : isMobile ? 767 : 1440,
      isMobile,
      isTablet: computed(() => !isMobile && mobileOrTablet),
      isMobileOrTablet: mobileOrTablet,
      isDesktop: computed(() => !isMobile && !mobileOrTablet),
      isFullHd: computed(() => !isMobile && !mobileOrTablet),
    })
  }

  nuxtApp.vueApp.config.globalProperties.$screen = screen
})
